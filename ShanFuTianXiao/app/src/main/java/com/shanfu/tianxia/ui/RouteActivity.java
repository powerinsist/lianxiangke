...
import com.shanfu.tianxia.adapter.BusResultListAdapter;
import com.shanfu.tianxia.overlay.DrivingRouteOverlay;
import com.shanfu.tianxia.overlay.WalkRouteOverlay;
import com.shanfu.tianxia.utils.AMapUtil;
...
public class RouteActivity extends Activity implements OnMapClickListener,
	...
	protected void onCreate(Bundle bundle) {
		...
		ly = getIntent().getStringExtra("lx");
		lx = getIntent().getStringExtra("ly");

		if(!TextUtils.isEmpty(lx)&&!TextUtils.isEmpty(ly)){
			mEndPoint = new LatLonPoint(Double.valueOf(ly), Double.valueOf(lx));
		}
...
//		getIntentData();
	...
	public void searchRouteResult(int routeType, int mode) {
		if (mStartPoint == null) {
			...
			TUtils.showShort(mContext, "起点未设置");
			return;
		}
		if (mEndPoint == null) {

			TUtils.showShort(mContext,"终点未设置");
		}
		showProgressDialog();
		...
		if (errorCode == AMapException.CODE_AMAP_SUCCESS) {
			if (result != null && result.getPaths() != null) {
				if (result.getPaths().size() > 0) {
					...
					TUtils.showShort(mContext, R.string.no_result);
				}
			} else {
				TUtils.showShort(mContext, R.string.no_result);
			...
			TUtils.showShort(this.getApplicationContext(), errorCode);
		...
		if (errorCode == AMapException.CODE_AMAP_SUCCESS) {
			if (result != null && result.getPaths() != null) {
				if (result.getPaths().size() > 0) {
					...
					//ToastUtil.show(mContext, R.string.no_result);
					//TUtils.show(mContext, R.string.no_result);
					TUtils.showShort(mContext, R.string.no_result);
				}
	...
	public void doFailSomething(){
		...
		//TUtils.showShort(this, "doFailSomething");
		finish();
	}
			...
			public void onLocationChanged(AMapLocation aMapLocation) {
				if (aMapLocation != null) {
					if (aMapLocation.getErrorCode() == 0) {
						...
						//定位失败
						TUtils.showShort(RouteActivity.this,"定位失败");
