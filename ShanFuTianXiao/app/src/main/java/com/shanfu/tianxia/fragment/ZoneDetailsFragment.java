...
public class ZoneDetailsFragment extends Fragment implements View.OnClickListener{
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        view = inflater.inflate(R.layout.layout_zone_details, null);
    ...
    private void initView() {
        details_tv = (WebView) view.findViewById(R.id.details_tv);
        lijiduihuan_tv = (TextView) view.findViewById(R.id.lijiduihuan_tv);
        lijiduihuan_tv.setOnClickListener(this);
        String weburl = "http://api.lianxiangke.cn/index.php?m=shop&c=shopdetail&a=goodsdescription&goods_id=";
        String url = weburl + goods_id;
        WebSettings settings = details_tv.getSettings();
        settings.setSupportZoom(true); // 支持缩放
        settings.setBuiltInZoomControls(true); // 启用内置缩放装置
        settings.setJavaScriptEnabled(true); // 启用JS脚本
        settings.setDefaultTextEncodingName("utf-8");
        details_tv.setWebViewClient(new WebViewClient() {
            @Override
