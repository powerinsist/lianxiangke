...
public class PagerSlidingTab extends HorizontalScrollView {
	...
	private int tabPadding = 30;//text的左右padding 30
	private int dividerWidth = 1;//分割线的宽度

	private int tabTextSize = 20;//字体的大小
	private int tabTextColor = 0xFF666666;//指示器字体的颜色
	...
	public void notifyDataSetChanged() {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			} else {//获取条目后动态创建TextView,获取pageTitle
	...
	private void updateTabStyles() {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (shouldExpand) {
				...
				//设置textView的padding
				v.setPadding(tabPadding,tabPadding/2, tabPadding,tabPadding/2);
			}
